...
public:
  ...
  int get_total_register_storage() {
    //Calculate total register storage as:
    //Each register takes 8 bytes.
    //Only registers V10+ need storage alloc (hence subtract 10).

    auto calc = (m_top-10)*8;
    return calc;
  }
